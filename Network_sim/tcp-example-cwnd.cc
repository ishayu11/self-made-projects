...
int main(int argc, char* argv[])
{
  ...
  cmd.AddValue("tcp_variant", "TcpNewReno or TcpCubic", tcp_variant);
  cmd.Parse(argc, argv);
  ...
  p2pBottleneck.SetQueue("ns3::DropTailQueue", "MaxSize", StringValue("1000p"));
  ...
  sourceApp1.Start(Seconds(1.0)); // Starts at 1.0s
  ...
  sourceApp2.Start(Seconds(1.0)); // Starts at 1.0s to compete simultaneously
  sourceApp2.Stop(Seconds(simulation_time));

  // 7. Tracing CWND to shared folder
  AsciiTraceHelper asciiTraceHelper;

  Ptr<OutputStreamWrapper> stream1 = asciiTraceHelper.CreateFileStream("scratch/my-projects/flow1.cwnd");
  Simulator::Schedule(Seconds(1.1), &TraceCwnd, "/NodeList/0/$ns3::TcpL4Protocol/SocketList/0/CongestionWindow", stream1);

  Ptr<OutputStreamWrapper> stream2 = asciiTraceHelper.CreateFileStream("scratch/my-projects/flow2.cwnd");
  ...
  cout << "\n=== Simulation Results (Competing Flows) ===\n";
  for (auto const& [id, stat] : stats) {
      ...
      // Only print the data flows (ignore the tiny ACK flows)
      if(throughput > 1.0) {
